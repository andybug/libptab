...
{
	static const char desc[] = "ptab (\"pretty table\") is a utility to "
	...
	try {
		...
		// unicode switch
		TCLAP::SwitchArg unicodeSwitch(
		    "u",
		    "unicode",
		    "Output using Unicode graph characters",
		    cmd,
		    false);
		...
		TCLAP::ValuesConstraint<std::string> formatConstraints(
		    format_list);
		...
		// align arg
		TCLAP::ValueArg<std::string> alignArg(
		    "",
		    "align",
		    "Manually set column alignment, one character per column."
		    "For example, a table with three columns could be set to "
		    "align"
		    "the first to columns on the left and the last column on "
		    "the"
		    "right like so: '--align llr'. If this is not provided, "
		    "ptab"
		    "will attempt to detect the data formats and align "
		    "appropriately.",
		    false,
		    "none",
		    "[l|r]+",
		...
		// delim arg
		TCLAP::ValueArg<std::string> delimArg(
		    "d",
		    "delim",
